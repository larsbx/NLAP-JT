...
| finite kneading letters of periodic ray addresses | `src/C1_residual_directive_carrier.mojo` (`KneadingLetters`, `ResidualDirectiveCarrier`) | CONFORMS | exact `Q` comparisons against the two cut points `theta/2` and `(theta+1)/2` on BigZ-backed addresses; period search bounded by an explicit cap, with every failure an explicit rejection; feeds the vendored tuning kernels and decides no certificate |
